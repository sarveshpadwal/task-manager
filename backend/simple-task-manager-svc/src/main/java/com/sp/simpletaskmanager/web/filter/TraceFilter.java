package com.sp.simpletaskmanager.web.filter;
...
public class TraceFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (!response.getHeaderNames().contains(TRACKING_HEADER)) {
            if (Optional.of(tracer).map(Tracer::currentSpan).map(Span::context).isEmpty()) {
    ...
    public Optional<String> getCurrentTrace() {
        try {
            ...
            return Optional.of(DEFAULT + "-" + traceId + "-" + parentId + "-" + DEFAULT);
        } catch (Exception ex) {
            log.error("error while fetching current trace", ex);
